...
- **Criteria**:
  - [x] `Ready to Assign` header banner updates live with color state (green/neutral/red).
  - [x] Category rows render progress bars and available balances from SQLite.
  - [x] Quick-pill buttons allow 1-tap envelope funding from unassigned pool.
  - [x] Visual indicators differentiate unfunded credit debt from liquid cash deficits.
...
- **Criteria**:
  - [x] `useCashflow` hook computes daily cumulative spend and blended EOM velocity.
  - [x] SVG trajectory curve plots actual spend through today with dashed EOM forecast.
  - [x] Horizontal Income Ceiling line renders with warning styling if spend crosses it.
  - [x] Titanium Hero Card metrics reflect live net cash flow and burn rate percentage.
...
- **Criteria**:
  - [x] Pan gesture tracks touch coordinates to calendar day boundaries.
  - [x] Micro-haptic ticks fire upon crossing day tick markers.
  - [x] Floating tooltip HUD displays scrubbed date, cumulative spend, and pace delta.
  - [x] Recent Outflows list dynamically filters to the scrubbed date during drag.
...
- **Criteria**:
  - [x] Month selector component with animated curve transitions.
  - [x] Historical queries load previous month ledger records cleanly.
  - [x] Rollover logic carries forward positive category balances and deducts cash deficits.
